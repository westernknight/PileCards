...
namespace SocketConnection
{
    public class SocketConnection
    {
        ...
        public Action disConnected_callback;
        public Action<string> cmd_callback;
        public Action<string> cmd_callback_mainThread;
        public Action<byte[]> cmd_bytes_callback;
        ...
        {
            if (cmd_callback_mainThread!=null)
            {
                if (messageQueue.Count>0)
                {
                    cmd_callback_mainThread(messageQueue[0]);
        ...
        protected void DealPackage(Socket ts, byte[] body_data)
        {
            ...
            if (cmd_callback_mainThread!=null)
            {
                messageQueue.Add(Encoding.UTF8.GetString(body_data));
            }
            else if (cmd_bytes_callback!=null)
            {
                cmd_bytes_callback(body_data);
            }
        }
        ...
        protected void ReceiveCallback(IAsyncResult result)
        {
            ...
            catch (Exception ex)
            {
                if (disConnected_callback != null)
                    disConnected_callback();
                Msg("socketClient.GetSocket().Connected" + (GetSocket().Connected ));
                Msg(ex);
            }
        }
